...
function isSessionValid() {
    // Faqat token borligini tekshiramiz — muddati yo'q
...
function createSession(username) {
    ...
    // localStorage — brauzer yopilsa ham qoladi
...
function showLoginPage() {
    ...
    overlay.innerHTML = `
        ...
        ">
            <style>
                ...
                .saved-creds-btn { width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:10px 14px;display:flex;align-items:center;gap:12px;cursor:pointer;transition:.18s;margin-bottom:12px;font-family:'Geist',-apple-system,sans-serif; }
                .saved-creds-btn:hover { background:rgba(255,255,255,0.08);border-color:rgba(59,130,246,0.3); }
                #otp-input { text-align:center;font-size:28px !important;font-weight:700 !important;letter-spacing:12px;padding:14px 14px !important; }
            ...
            ${savedCreds && hasUsers && isDeviceTrusted(savedCreds.username) ? `
            ...
            <div id="saved-alt-row" style="text-align:center;margin-bottom:14px">
            ...
            <div id="auth-manual-form" style="display:${(savedCreds && hasUsers && isDeviceTrusted(savedCreds.username)) ? 'none' : 'flex'};flex-direction:column;gap:14px">
                <div>
                ...
                <div style="text-align:center;margin-top:4px">
                    ${hasUsers
                        ? `<span style="font-size:13px;color:#475569">Yangi hisob? </span><button id="auth-toggle" onclick="toggleAuthMode()">Ro'yxatdan o'tish</button>`
                        : `<span style="font-size:13px;color:#475569">Hisob bor? </span><button id="auth-toggle" onclick="toggleAuthMode()">Kirish</button>`
                    }
                </div>
            </div>
            ...
            <div id="auth-otp-form" style="display:none;flex-direction:column;gap:16px">
                ...
                <button id="otp-verify-btn" onclick="handleOtpVerify()" style="width:100%;background:#3b82f6;color:#fff;border:none;border-radius:10px;padding:12px;font-size:14px;font-weight:600;cursor:pointer;font-family:'Geist',-apple-system,sans-serif;display:flex;align-items:center;justify-content:center;gap:8px;">
    ...
    // Agar foydalanuvchi yo'q bo'lsa — register rejimi, bor bo'lsa — login
...
function handleOtpVerify() {
    ...
    if (result === 'ok') {
        ...
        if (passEl && passEl.value) saveCreds(_pendingUsername, passEl.value);
        else {
            const creds = getSavedCreds();
            if (creds && creds.username === _pendingUsername) saveCreds(_pendingUsername, creds.password);
        }
        createSession(_pendingUsername);
...
function cancelOtp() {
    clearInterval(_otpCountdownInterval);
    clearInterval(_resendCountdownInterval);
    _otpCode = null; _pendingUsername = null; _otpExpiry = null;
    const otpForm = document.getElementById('auth-otp-form');
    const manualForm = document.getElementById('auth-manual-form');
...
function renderProfilePage() {
    ...
    document.getElementById('content').innerHTML = `
        <style>
            ...
            .profile-input-wrap input { width:100%;background:rgba(0,0,0,0.3);border:1px solid var(--border2);border-radius:8px;color:var(--text1);padding:10px 14px;font-size:13px;outline:none;transition:.18s;font-family:var(--font);box-sizing:border-box; }
            .profile-input-wrap input:focus { border-color:var(--accent);box-shadow:0 0 0 3px rgba(59,130,246,0.12); }
        ...
        <div class="profile-page">
            <div class="profile-section">
                ...
                <div style="display:flex;align-items:center;gap:20px;margin-bottom:20px">
                    ...
                    <div>
                        ...
                        <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap;">
                            <button onclick="document.getElementById('avatar-upload').click()" style="background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.2);color:#3b82f6;border-radius:7px;padding:5px 12px;font-size:12px;font-weight:600;cursor:pointer;font-family:var(--font);display:flex;align-items:center;gap:5px;"
                ...
                <div class="pm-success" id="pf-success" style="margin-bottom:12px"><i class="fas fa-check-circle"></i><span></span></div>
            ...
            <div class="profile-section">
                <div class="profile-section-title">Xavfsizlik</div>
                <div style="font-size:13px;color:var(--text2);margin-bottom:14px">Joriy qurilma ishonchli sifatida saqlangan. Shu qurilmadan kirganda Telegram kodi so'ralmaydi.</div>
                <button class="pf-btn" onclick="removeTrustedDevice()" style="background:rgba(245,158,11,0.1);color:#f59e0b;border:1px solid rgba(245,158,11,0.25);width:auto;padding:10px 24px;" onmouseover="this.style.background='#f59e0b';this.style.color='#fff'" onmouseout="this.style.background='rgba(245,158,11,0.1)';this.style.color='#f59e0b'">
            ...
            <div class="profile-section">
                ...
                <div id="pf-add-user-form" style="display:none;margin-top:14px;background:rgba(0,0,0,0.2);border-radius:10px;padding:14px">
                    <div class="profile-grid">
                        <div class="profile-input-wrap" style="margin-bottom:0"><label>Login</label><input type="text" id="pf-new-login" placeholder="Foydalanuvchi nomi"></div>
                        <div class="profile-input-wrap" style="margin-bottom:0"><label>Parol</label><input type="password" id="pf-new-pass2" placeholder="Parol"></div>
                    </div>
                    <div style="margin-top:10px">
                        <div class="profile-input-wrap" style="margin-bottom:10px"><label>Tasdiqlang</label><input type="password" id="pf-new-confirm2" placeholder="Qaytaring"></div>
...
function removeTrustedDevice() {
    ...
    alert("Qurilma ishonchi olib tashlandi.");
}
...
function deleteAvatar() {
    ...
    const currentUser = getCurrentUser();
    const preview = document.getElementById('profile-avatar-preview');
    if (preview) preview.innerHTML = (profile.name || currentUser || 'U')[0].toUpperCase();
    updateTopbarAvatar();
}
...
function changePasswordFromPage() {
    ...
    if (!checkLogin(currentUser, oldPass)) { errEl.querySelector('span').textContent = "Joriy parol noto'g'ri!"; errEl.style.display = 'flex'; return; }
